...
=== תהליך שיפור פנימי (אל תוציא לפלט) ===

לפני הפלט הסופי, עבור באופן פנימי ושקט על הצעדים הבאים — אל תכתוב אותם בתשובה:
1. כתוב טיוטה לפי כל הכללים.
2. קרא אותה בעיני {target} שנכווה מיועצים: באיזה משפט הפסקת? מה הרגיש כקלישאה?
3. תקן רק שוברי אמון. אל תיגע בהיפוכים ומטאפורות — ריכוך = בינוני.
...
def _clean_post(raw: str) -> str:
    resp = client.messages.create(
        ...
        max_tokens=1800,
        messages=[{"role": "user", "content": f"""הטקסט שלהלן מכיל תהליך כתיבה עם סעיפי "תפקיד 1", "תפקיד 2", "תפקיד 3", ביקורת ותיקונים.

המשימה: החזר אך ורק את גוף הפוסט הסופי.

כללים מוחלטים:
- אל תכלול שום שורה שמכילה את המילה "תפקיד"
- אל תכלול שאלות ביקורת (1. 2. 3.)
- אל תכלול הסברים על שינויים או תיקונים
- הסר ** מכל מקום
- הסר ## מכל מקום
- החזר רק את טקסט הפוסט הסופי, ממשפט הפתיחה ועד לסוף
